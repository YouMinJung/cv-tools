...
def get_images_info_and_annotations(opt):
    ...
    for file_path in file_paths:
        ...
        image_annotation = create_image_annotation(file_path=file_path, width=w, height=h, image_id=image_id, dataset_root=opt.data_path)
        images_annotations.append(image_annotation)

        label_file_name = f"{file_path.stem}.txt"
        if opt.yolo_subdir:
            annotations_path = file_path.parent / YOLO_SUB_DIR / label_file_name
        else:
        ...
        if annotations_path.exists():
            ...
            for line1 in label_read_line:
                ...
                # yolo result to COCO result (saves confidence)
                if opt.results == True:
...
def get_args():
    ...
    parser.add_argument(
        "--data-path",
        type=str,
        help="Absolute path for the dataset root dir"
    )
    parser.add_argument(
        "--debug",
        action="store_true",
    ...
    parser.add_argument(
        "--box2seg",
        action="store_true",
        help="COCO segmentation will be populated with a polygon that matches replacates the bounding box data.",
    )
    parser.add_argument(
        "--results",
        action="store_true",
...
def main(opt):
    ...
    output_path = 'output_' + output_name
